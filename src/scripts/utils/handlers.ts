export function handleLikeButtons() {
  ...
  likeButtons.forEach((likeButton) => {
    likeButton.addEventListener('click', (event) => {
      ...
      const sameCards = document.querySelectorAll(`[data-card-id="${parentElementDatasetId}"]`);
      ...
      if (savedLikesArray.includes(parentElementDatasetId)) {
        savedLikesArray.splice(savedLikesArray.indexOf(parentElementDatasetId), 1);
      } else {
...
export function handleCategoryCardClick() {
  ...
  categoriesButton.addEventListener('click', () => {
    ...
    const categoriesDetails = document.getElementById('categories__details') as HTMLDetailsElement;
