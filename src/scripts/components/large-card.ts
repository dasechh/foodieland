...
export class largeCard {
  ...
  private static template = `
          <div class="card feed__card">
        <img class="card__image" />
        <div class="feed__card-like-button like-button">
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
  ...
  constructor(private options: smallCardData) {
    this.divElement = this.createCard();
  }

  private createCard(): HTMLDivElement {
    const tempDiv = document.createElement("div");
    tempDiv.innerHTML = largeCard.template;
    const div = tempDiv.querySelector("div") as HTMLDivElement;

    div.dataset.cardId = this.options.id.toString();


    (div.querySelector(".card__image") as HTMLImageElement).src = this.options.imgSrc // Здесь повнимательнее нужно быть

    const cardTitle = div.querySelector(".card__title") as HTMLHeadingElement;
    cardTitle.textContent = this.options.name;

    const tagsWrapper = div.querySelector("card__tags");
