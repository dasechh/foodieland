...
export async function displayCards(
  ...
  try {
    ...
    images.forEach((data, index) => {
      const cardElement = createCard(data, templateId, cardClass, imageClass, titleClass, tagsClass);
...
async function loadImages(fileName: string) {
  return new Promise<{ imgSrc: string; name: string; tags: string[]; id: number }[]>((resolve, reject) => {
    fetch(fileName)
      .then((response) => {
        if (!response.ok) {
...
function createCard(
  ...
  const recipeCardTemplate = document.getElementById(templateName) as HTMLTemplateElement;
  const newCard = recipeCardTemplate.content.cloneNode(true) as DocumentFragment;
  const cardElement = newCard.querySelector(`.${cardElementClass}`) as HTMLElement;

  const cardImage = cardElement.querySelector(`.${cardImageClass}`) as HTMLImageElement;
  cardImage.src = data.imgSrc;

  const cardTitle = cardElement.querySelector(`.${cardTitleClass}`) as HTMLElement;
  cardTitle.textContent = data.name;

  const cardTags = cardElement.querySelector(`.${cardTagsClass}`) as HTMLElement;
